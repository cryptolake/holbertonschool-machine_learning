...
def availableShips(passengerCount):
    """Return the list of ships that hold given number of passengers."""
    starships_request = requests\
        .get('https://swapi-api.hbtn.io/api/starships/')\
        .json()
    ...
    available_starships = filter(lambda x: x['passengers'].replace(',', '')
                                 .isdigit() and int(x['passengers']
                                                    .replace(',', ''))
                                 >= passengerCount, starships)
